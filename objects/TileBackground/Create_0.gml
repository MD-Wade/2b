...
function draw_background_normal() {
    image_blend = merge_colour(c_black, c_maroon, wave(0.2, 1, 16, 0));
	draw_sprite_tiled_ext(sprite_index, image_index, 0, 0, image_xscale, image_yscale, image_blend, image_alpha);
}
...
album_sprite = global.track_details_current.dynamic_sprite;
var _sprite_width = sprite_get_width(album_sprite);
var _sprite_height = sprite_get_height(album_sprite);
...
album_scaled_width = _sprite_width * _scale_x;
album_scaled_height = _sprite_height * _scale_y;
album_offset_x = (room_width - album_scaled_width) / 2;
album_offset_y = (room_height - album_scaled_height) / 2;
